...
function Header() {
  ...
  return (
    <header className="bg-theme-blue">
      ...
      >
        ...
        <div className="flex lg:hidden">
          <button
            ...
            className="-m-2.5 iline-flex items center justify-center rounded p-2.5 text-white"
          >
                  ...
                  {products.map((item) => (
                    ...
                    >
                      <div className="flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-gray-200">
                        <item.icon className="h-6 w-6 text-theme-blue group-hover:text-blue-600" />
                      </div>
                      <div className="flex-auto">
                        <a
                          href={item.href}
                          className="block font-semibold text-theme-blue"
                        >
                          {item.name}
                          <span className="absolute inset-0" />
                        </a>
                        <p className="mt-1 text-theme-blue">
                          {item.description}
                        </p>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
                  {callsToAction.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="flex items-center justify-center gap-x-2.5 p-3 text-sm font-semibold leading-6 text-theme-blue hover:bg-gray-100"
                    >
                      <item.icon
                        className="h-5 w-5 flex-none text-theme-blue"
                        aria-hidden="true"
                      />
                      {item.name}
                    </a>
      ...
      >
        <div className="fixed inset z-10" />
        <Dialog.Panel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-theme-blue px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
